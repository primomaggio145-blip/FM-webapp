      ...
      try {
        ...
        if (window.__FM_RELOAD__) {
          const reloadData = {
            ...
            spese:    (sP||[]).map(r => ({ id:String(r.id), docenteId:r.docente_id||null, data:r.data||'', mese:r.mese!=null?r.mese:new Date((r.data||'')+'T00:00:00').getMonth(), anno:r.anno||new Date().getFullYear(), importo:parseFloat(r.importo)||0, desc:r.desc||r.descrizione||'', nota:r.nota||'', note:r.note||'', metodo:r.metodo||'Bonifico', categoria:r.categoria||'altro', haAcconto:r.ha_acconto||false, accontoImporto:r.acconto_importo!=null?parseFloat(r.acconto_importo):null, importoLordo:r.importo_lordo!=null?parseFloat(r.importo_lordo):null })),
