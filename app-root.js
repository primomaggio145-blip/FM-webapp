...
function App() {
        ...
        if (window.__FM_RELOAD__) {
          const reloadData = {
            ...
            brani:    (sB||[]).map(r => {
              ...
              return { id:r.id, title:r.titolo||'', composer:r.compositore||'', tipo:r.tipo||'individuale', strumento:r.strumento||'', genere:r.genere||'' /* [FM-BRANO-DA-LEZIONE] prima perso al refresh */, eventiIds:pj(r.eventi_ids,[]), versioni:versioni, note:r.note||'', lezioni:r.lezioni||0, periodo:r.periodo||'', difficulty:r.difficolta||'Intermedio' };
            }),
            spese:    (sP||[]).map(r => ({ id:String(r.id), docenteId:r.docente_id||null, data:r.data||'', mese:r.mese!=null?r.mese:new Date((r.data||'')+'T00:00:00').getMonth(), anno:r.anno||new Date().getFullYear(), importo:parseFloat(r.importo)||0, desc:r.desc||r.descrizione||'', nota:r.nota||'', note:r.note||'', metodo:r.metodo||'Bonifico', categoria:r.categoria||'altro', haAcconto:r.ha_acconto||false, isAcconto:r.is_acconto===true, accontoImporto:r.acconto_importo!=null?parseFloat(r.acconto_importo):null, importoLordo:r.importo_lordo!=null?parseFloat(r.importo_lordo):null })),
            entrate:  (sQ||[]).map(r => { const smQ={'da pagare':'attesa','in ritardo':'ritardo'}; return { id:String(r.id), studentId:r.studente_id||null, studentName:r.studente_nome||'', importo:parseFloat(r.importo)||0, mese:r.mese, anno:r.anno, data:r.data_pagamento||'', dataPagamento:r.data_pagamento||'', metodo:r.metodo||'Contanti', categoria:r.categoria||'quota', desc:r.descrizione||'', note:r.note||'', numRicevuta:r.num_ricevuta||'', noRicevuta:r.no_ricevuta===true, stato: smQ[r.stato]||r.stato||'attesa' }; }),
            concerti: (sEV||[]).map(r => {
              const pj = (v,f=[]) => { if(!v) return f; if(Array.isArray(v)) return v; if(typeof v==='object') return v; try { return JSON.parse(v); } catch(e) { return f; } };
  ...
  const renderCurrentView = () => {
    switch(view) {
      ...
      case 'cestino':     return React.createElement(CestinoView, { userRuolo: user?.ruolo||"admin" });
