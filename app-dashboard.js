...
const DashboardView = ({ appUser, onNavigate, config:propConfig, setConfig:propSetConfig, anniScolastici:propAnni, setAnniScolastici:propSetAnni, students:propStudentsDash, entrate:propEntrateDash, spese:propSpeseDash, docenti:propDocentiDash, lessons:propLessonsDash, concerti:propConcertiDash, richieste:propRichieste, notifiche:propNotifiche, setNotifiche:propSetNotifiche, panels:propPanels, setPanels:propSetPanels, iscrizioniAnno:propIscrizioniAnnoDash, onQuickAction, temaAttivo, setTemaAttivoPreview, sfondoTeen, salvaTemaDispositivo }) => {
  ...
  useEffect(() => {
    ...
    const matchLezioneAllievo = (l) => {
      ...
      // record/collegamento — hanno contactName invece di student/studentId. Richiede
      // SEMPRE il nome completo (mai solo il primo nome): un confronto parziale
      // rischierebbe di mostrare a un allievo le lezioni di un altro con nome simile.
      const cn = (l.contactName || "").toLowerCase().trim();
      if (myStudentName && cn && cn === myStudentName.toLowerCase().trim()) return true;
            ...
            , React.createElement('div', null
              ...
              , ruolo==="docente" ? React.createElement(React.Fragment, null
                  ...
                  , React.createElement(KpiCard, { icon: "euro", label: "Compenso mese",
                      ...
                      sub: "mese corrente", hex: C.green, hideAmounts: !showAmounts, onClick: () => { onQuickAction && onQuickAction('showCompenso'); onNavigate('docenti'); }})
                )
